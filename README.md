...
Todas las preguntas están en `js/preguntas.js`, organizadas por rondas
(actualmente 6 rondas, ~90 preguntas en total). Puedes editar textos, agregar
preguntas nuevas o cambiar el tiempo de cada una sin tocar el resto del
código — solo respeta el formato de cada objeto.

**Importante:** cada partida NO usa las 90 preguntas. Al crear la sala, el
juego elige al azar `PREGUNTAS_POR_PARTIDA` (10 por defecto, configurable al
final de `js/preguntas.js`) y las juega en orden aleatorio. Así cada partida
es distinta y las preguntas no se memorizan de una clase a otra. Para que el
banco dure más partidas sin repetirse tanto, lo ideal es seguir sumando
preguntas nuevas dentro de cada ronda con el tiempo.
